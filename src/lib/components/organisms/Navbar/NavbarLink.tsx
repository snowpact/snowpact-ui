...
export const NavbarLink: FC<NavbarLinkProps> = ({
  ...
  return (
    <li className="group relative w-full list-none whitespace-nowrap text-center md:w-fit md:text-left">
      <LinkOrDivComponent
        href={linkAsTo ? undefined : href}
        to={linkAsTo && href ? href : undefined}
        className={twMerge(
          twJoin(
            'block pr-4 pl-3 md:p-0 md:py-0 py-2 mx-auto',
            theme.base,
            active && cs('bg-primary-700 text-white md:bg-transparent md:text-primary-700', theme.active),
            disabled && cs('text-gray-400 hover:cursor-not-allowed', theme.disabled)
          )
        )}
        {...props}
